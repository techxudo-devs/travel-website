...
export default function ActivitiesExtrasSection({ activities = [], extras = [] }) {
        ...
        >
            <div className="container mx-auto px-4">
                ...
                <motion.div variants={itemVariants} className="text-center mb-12">
                    <h2 className="text-4xl font-bold text-first">Activities & Experiences</h2>
                    <p className="mt-2 text-lg text-dark max-w-2xl mx-auto">
                        Here's a glimpse of the thrilling activities included and optional extras to enhance your journey.
                    </p>
                ...
                <div className="grid md:grid-cols-2 gap-12">
                    ...
                    {activities.length > 0 && (
                        <motion.div variants={sectionVariants}>
                            <h3 className="text-2xl font-bold text-second mb-6">Activities Included</h3>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {activities.map((activity) => (
                                    <motion.div key={activity._id} variants={itemVariants}>
                                        <FeatureCard name={activity.name} icon={<FaHiking size={24} />} />
                                    </motion.div>
                                ))}
                            </div>
                        </motion.div>
                    ...
                    {extras.length > 0 && (
                        <motion.div variants={sectionVariants}>
                            <h3 className="text-2xl font-bold text-brown mb-6">Optional Extras</h3>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {extras.map((extra) => (
